...
viz_ags_params = {'OwnPCA': {'X':X, 'data_name':args.dataset, 'k':args.components},
                 'PCA': {'data':X, 'data_name':args.dataset},
                 'iPCA':{'data':X, 'data_name':args.dataset},
                 'Isomap':{'data':X, 'data_name':args.dataset}}
...
if args.experiment == 'dr':
    ...
    dim_red.visualize(Y, layout=(2,5) ,axes=range(4),exclude=['4d'], figsize=(30,15), title_size=8)
    dim_red.visualize(Y,  layout=(2,5), axes=range(4),data2plot='Transformed', exclude=['4d'], figsize=(30,15), title_size=8)
