...
class Dataset():
    ...
    def standardization(df: pd.DataFrame, method, cat_transf, wmean=True, wstd=True, num_cat=2):
        ...
        if method == 'categorical' and len(num_features) != 0:
            try:
                processed_df[processed_df.select_dtypes(exclude='object').columns] = processed_df.select_dtypes(
                    exclude='object').apply(lambda x: pd.qcut(x, num_cat, labels=False))
